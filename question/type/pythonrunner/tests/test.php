...
/**
 * Base testcase for pythonrunner tests.
...
class qtype_pythonrunner_testcase extends advanced_testcase {
    ...
    public static function setup_test_sandbox_configuration(): void {
        global $CFG, $USER;
        require($CFG->dirroot . '/question/type/pythonrunner/tests/fixtures/test-sandbox-config-dist.php');
        $localconfig = $CFG->dirroot . '/question/type/pythonrunner/tests/fixtures/test-sandbox-config.php';
    ...
    protected function check_language_available($language): void {
        if (qtype_pythonrunner_sandbox::get_best_sandbox($language, true) === null) {
    ...
    public function make_question($question) {
        try {
            $q = test_question_maker::make_question('pythonrunner', $question);
        } catch (qtype_pythonrunner_missing_question_type $ex) {
    ...
    protected function check_sandbox_enabled($sandbox): void {
        if (!get_config('qtype_pythonrunner', $sandbox . '_enabled')) {
