...
/**
 * Unit tests for the pythonrunner question type.
 *
...
class qtype_pythonrunner_walkthrough_testcase extends qbehaviour_walkthrough_test_base {
    ...
    public function test_adaptive() {
        $q = test_question_maker::make_question('pythonrunner', 'sqr');
        ...
        $this->check_current_output(
                new question_pattern_expectation('/' .
                        preg_quote(get_string('noerrorsallowed', 'qtype_pythonrunner') . '/'))
              );
    ...
    public function test_view_hidden_testcases_capability() {
        ...
        $this->assertTrue(has_capability('qtype/pythonrunner:viewhiddentestcases', $coursecontext));
        ...
        role_change_permission($DB->get_field('role', 'id', ['shortname' => 'editingteacher']),
                $coursecontext, 'qtype/pythonrunner:viewhiddentestcases', CAP_PREVENT);
        $this->assertFalse(has_capability('qtype/pythonrunner:viewhiddentestcases', $coursecontext));
    ...
    public function test_partial_marks() {
        $q = test_question_maker::make_question('pythonrunner', 'sqr_part_marks');
        $this->start_attempt_at_question($q, 'adaptive', 1, 1);
    ...
    public function test_grading_template_output() {
        $q = test_question_maker::make_question('pythonrunner', 'sqrnoprint');
