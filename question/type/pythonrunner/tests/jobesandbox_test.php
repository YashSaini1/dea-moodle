...
/**
 * Unit tests for pythonrunner's jobe sandbox class.
 *
...
class qtype_pythonrunner_jobesandbox_testcase extends qtype_pythonrunner_testcase {

    public function test_fail_with_bad_key() {
        $this->check_sandbox_enabled('jobesandbox');
        if (!get_config('qtype_pythonrunner', 'jobe_apikey_enabled')) {
            $this->markTestSkipped("Jobe API key security disabled: test skipped");
        }
        set_config('jobe_apikey', 'no-such-key-we-hope', 'qtype_pythonrunner');
        $sandbox = new qtype_pythonrunner_jobesandbox();
    ...
    public function test_jobesandbox_python3_good() {
        ...
        $this->assertEquals(qtype_pythonrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_pythonrunner_sandbox::RESULT_SUCCESS, $result->result);
    ...
    public function test_jobesandbox_python3_bad() {
        ...
        $result = $sandbox->execute($source, 'python3', '');
        $this->assertEquals(qtype_pythonrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_pythonrunner_sandbox::RESULT_COMPILATION_ERROR, $result->result);
        $sandbox->close();
    }
...
";
    ...
    public function test_jobesandbox_python3_timeout() {
        ...
        $result = $sandbox->execute($source, 'python3', '');
        $this->assertEquals(qtype_pythonrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_pythonrunner_sandbox::RESULT_TIME_LIMIT, $result->result);
...
EOCODE;
    ...
    public function test_limits_enforced() {
        $this->check_sandbox_enabled('jobesandbox');
        if (!get_config('qtype_pythonrunner', 'jobe_limits_enforced')) {
