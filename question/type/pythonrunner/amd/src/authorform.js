 ...
 *
 * @module qtype_pythonrunner/authorform
 * @copyright  Richard Lobb, 2015, The University of Canterbury
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

define(['jquery', 'qtype_pythonrunner/userinterfacewrapper', 'core/str'], function($, ui, str) {
    ...
    function initEditForm() {
        var typeCombo = $('#id_pythonrunnertype'),
            ...
            pythonType = $('#id_pythontype'),
            sql_queries = $('#id_sql_queries_header'),
        ...
        function setUis() {
            ...
            if (enableUi) {
                ...
                if (document.querySelector('#id_sql_query')){
                    setUi('id_sql_query', uiname);
                }
         ...
         * set up by code in edit_pythonrunner_form.php (q.v.) but those event
        ...
        function copyFieldsFromQuestionType(newType, response) {
            ...
            str.get_string('pythonrunner_question_type', 'qtype_pythonrunner').then(function (s) {
         ...
         * @param {string} questionType The pythonrunner (sub) question type.
        ...
        function reportError(questionType, error) {
            ...
            str.get_string('prototype_error', 'qtype_pythonrunner').then(function(s) {
                var errorMessage = s + "\n";
                errorMessage += error + '\n';
         ...
         * @param {string} pythonrunner_descr The language string to introduce
         * the detail.
         ...
         */
        function detailsHtml(title, pythonrunner_descr, html) {

            var resultHtml = '<p class="question-type-details-header">';
            resultHtml += pythonrunner_descr;
            resultHtml += title + '</p>\n' + html;
            return resultHtml;
        ...
        function langStringAlert(key, extra) {
            ...
            str.get_string(key, 'qtype_pythonrunner').then(function(s) {
         ...
         * pythonrunner question type currently selected by the combobox.
         */
        function loadCustomisationFields() {
            ...
            if (newType !== '' && newType !== 'Undefined') {
                ...
                $.getJSON(M.cfg.wwwroot + '/question/type/pythonrunner/ajax.php',
                    {
                        qtype: newType,
                        courseid: courseId,
                    ...
                    str.get_string('ajax_error', 'qtype_pythonrunner').then(function(s) {
         ...
         * 'Precheck' selector.
         */
        function setup_python_type() {
            if (!pythonType || !sql_queries){
                return;
            }
            ...
            const update_state = () => {
                if (parseInt(pythonType.val()) === PYTHON_OTHER){
                    sql_queries.css('display', 'block');
                } else {
                    sql_queries.css('display', 'none');
                }
            };
        ...
        function checkForBrokenQuestion() {
            ...
            if (brokenQuestionMessage !== '') {
                ...
                $('#id_qtype_pythonrunner_error_div').append(messagePara);
            }
        }
        ...
        if (prototypeType.prop('value') == 1) {
            ...
            str.get_string('proceed_at_own_risk', 'qtype_pythonrunner').then(function(s) {
                alert(s);
            });
        ...
        if (!isCustomised) {
            ...
            str.get_string('info_unavailable', 'qtype_pythonrunner').then(function(s) {
        ...
        customise.on('change', function() {
            ...
            if (isCustomised) {
                ...
                str.get_string('confirm_proceed', 'qtype_pythonrunner').then(function(s) {
                    if (window.confirm(s)) {
                        setCustomisationVisibility(false);
                    } else {
        ...
        typeCombo.on('change', function() {
            if (customise.prop('checked')) {
                ...
                str.get_string('question_type_changed', 'qtype_pythonrunner').then(function (s) {
                    if (window.confirm(s)) {
        ...
        $('button.replaceexpectedwithgot').click(function() {
            let rownum = $(this).attr('data-order');
            let gotPre = $('pre#id_got_' + rownum);
            let expected = $('#id_expected_' + rownum);
            expected.val(gotPre.html())
            $('.failrow_' + rownum).addClass('fixed');  // Fixed row.
            $(this).prop('disabled', true);
        });

        document.querySelectorAll('button.replace_templateparams_with_sqlgot').forEach((elem) => {
            let element = $(elem);
            let rownum = element.attr('data-order');
            var gotPre = element.prev('#id_sqlgot_' + rownum);
            let expected_result = gotPre.text();
            let textarea = $('#id_sqlquery_'+rownum)
            element.click(() => {
                $('.sqlfailrow_' + rownum).addClass('fixed');  // Fixed row.
                $('input[name="sqlexpected['+rownum+']"').val(expected_result)
                $('input[name="lastcode['+rownum+']"').val(textarea.val())
