...
define(['jquery'], function ($) {
    ...
    function loadQuestionText(qid, divId, questionFilename) {
        ...
        qDiv = '<div>\n' +
            '<button type="button" class="qtype_pythonrunner_previous">Previous</button>\n' +
            '<button type="button" class="qtype_pythonrunner_next">Next</button>' +
            '&nbsp; &nbsp;' +
            '<span>Page: <span class="qtype_pythonrunner_pagenum"></span> / <span ' +
            'class="qtype_pythonrunner_numpages"></span></span>\n' +
            '</div>\n' +
            '<canvas id="qtype_pythonrunner_problemspec_' + qid + '"></canvas>\n' +
            '</div>\n';
        questionTextDiv.append(qDiv);
        canvas = document.getElementById('qtype_pythonrunner_problemspec_' + qid);
        next = $('#' + divId + ' button.qtype_pythonrunner_next');
        previous = $('#' + divId + ' button.qtype_pythonrunner_previous');
        pageNumSpan = $('#' + divId + ' span.qtype_pythonrunner_pagenum');
        numPagesSpan = $('#' + divId + ' span.qtype_pythonrunner_numpages');
        ctx = canvas.getContext('2d');
        next.click(onNextPage);
        previous.click(onPrevPage);
        $.getJSON(M.cfg.wwwroot + '/question/type/pythonrunner/problemspec.php',
                {
                    questionid: qid,
