...
// Initialise $PAGE.
$PAGE->set_url('/question/type/pythonrunner/questiontestrun.php', $urlparams);
$title = pythonrunner_str('testingquestion', format_string($question->name));
...
$exportquestionlink = new moodle_url('/question/type/pythonrunner/exportone.php', $urlparams);
$exportquestionlink->param('sesskey', sesskey());

// Create the question usage we will use.

$quba = question_engine::make_questions_usage_by_activity('qtype_pythonrunner', $context);
...
echo $OUTPUT->heading(sqlrunner_str('questionpreview'), 3);

echo html_writer::tag('p', html_writer::link($questionbanklink,
    sqlrunner_str('seethisquestioninthequestionbank')));

if ($canedit) {
    echo html_writer::tag('p',
            html_writer::link($exportquestionlink, pythonrunner_str('exportthisquestion')) .
            $OUTPUT->help_icon('exportthisquestion', 'qtype_pythonrunner'));
}
