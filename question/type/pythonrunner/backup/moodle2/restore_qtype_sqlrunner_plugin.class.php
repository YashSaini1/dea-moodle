...
/**
 * Restore plugin class for pythonrunner questions.
 */
class restore_qtype_pythonrunner_plugin extends restore_qtype_plugin {
    ...
    private function add_question_options(&$paths) {
        ...
        $elename = 'pythonrunner_options';
        $elepath = $this->get_pathfor('/pythonrunner_options/pythonrunner_option');  // We used get_recommended_name() so this works.
    ...
    private function add_question_testcases(&$paths) {
        ...
        $elename = 'pythonrunner_testcases';
        $elepath = $this->get_pathfor('/pythonrunner_testcases/pythonrunner_testcase');
        $paths[] = new restore_path_element($elename, $elepath);
    }
     ...
     * backup element.
     */
    public function process_pythonrunner_testcases($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            // Insert record.
            $newitemid = $DB->insert_record("question_python_tests", $data);
        }
     ...
     * backup element.
     */
    public function process_pythonrunner_options($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            // Insert the record.
            $newitemid = $DB->insert_record("question_python_options", $data);
        }
        // Nothing to remap if the question already existed.
