 ...
 * question type by looking up the prototype in the question_python_options
...
if ($qtype) {
    $questiontype = qtype_pythonrunner::get_prototype($qtype, $coursecontext);
    ...
    if (!in_array($uiplugin, $allnames)) {
        $uiheader = sqlrunner_str('unknownuiplugin', array('pluginname' => $uiplugin));
    } else {
        $uiparams = $uiplugins->parameters($uiplugin);
        if ($uiparams->length() === 0) {
            $uiheader = pythonrunner_str('nouiparameters', array('uiname' => $uiplugin));
        } else {
            $csv = implode(', ', $uiparams->all_names_starred());
            $uiheader = sqlrunner_str('uiparametertablehead', array('uiname' => $uiplugin)) . $csv . '.';
            $uiparamstable = $uiparams->table();
            $namehdr = sqlrunner_str('uiparamname');
            $descrhdr = sqlrunner_str('uiparamdesc');
            $defaulthdr = sqlrunner_str('uiparamdefault');
    ...
    echo json_encode(array('header' => $uiheader,
        ...
        'showdetails' => pythonrunner_str('showdetails'),
        'hidedetails' => pythonrunner_str('hidedetails')));
}
die();
