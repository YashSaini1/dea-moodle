 ...
 * pythonrunner questions in a given courses.
 *
...
$PAGE->set_url('/question/type/pythonrunner/findduplicatesindex.php');
...
foreach ($questionsbycontext as $contextid => $numpythonrunnerquestions) {
    ...
    if (has_capability('moodle/question:editall', $context)) {
        $availablequestionsbycontext[$contextid] = $numpythonrunnerquestions;
    }
}
...
if (count($availablequestionsbycontext) == 0) {
    ...
    foreach ($availablequestionsbycontext as $contextid => $numpythonrunnerquestions) {
        $context = context::instance_by_id($contextid);
        $name = $context->get_context_name(true, true);
        if (strpos($name, 'Course:') === 0) {
            $class = 'bulktest pythonrunner context quiz';
            $findduplicatesurl = new moodle_url('/question/type/pythonrunner/findduplicates.php', array('contextid' => $contextid));
            $findduplicateslink = html_writer::link($findduplicatesurl,
                    'Find duplicates',
                    array('title' => 'Find all duplicates in this context',
                          'style' => $buttonstyle));
            $litext = $name . ' (' . $numpythonrunnerquestions . ') ' . $findduplicateslink;
