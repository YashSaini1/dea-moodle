...
/** Defines a subclass of the normal pythonrunner testing_outcome for use when
...
class qtype_pythonrunner_combinator_grader_outcome extends qtype_pythonrunner_testing_outcome {
     ...
     * @param qtype_pythonrunner_question $q The question being rendered (ignored)
     * @return A table of test results. See the parent class for details.
     */
    public function get_test_results(qtype_pythonrunner_question $q) {
                ...
                for ($col = 0; $col < count($row); $col++) {
                    ...
                    if (in_array($columnheaders[$col], array('ishidden', 'iscorrect'))) {
                        ...
                        if ($formats[$formatindex++] === '%h') {
                            $newrow[] = new qtype_sqlrunner_html_wrapper($cell);
                        } else {
                            $newrow[] = $cell;
                        }
                    }
    ...
    private function validate_table_formats() {
        if ($this->columnformats && $this->testresults) {
            ...
            if (count($this->columnformats) !== $numcols) {
                $error = pythonrunner_str('wrongnumberofformats', array('expected' => $numcols, 'got' => count($this->columnformats)));
                $this->set_status(self::STATUS_BAD_COMBINATOR, $error);
            } else {
                foreach ($this->columnformats as $format) {
                    if ($format !== '%s' && $format !== '%h') {
                        $error = pythonrunner_str('illegalformat', array('format' => $format));
                        $this->set_status(self::STATUS_BAD_COMBINATOR, $error);
                        break;
                    }
                }
