...
/*
 * qtype_pythonrunner external file. Allows webservice access by authenticated
...
namespace qtype_pythonrunner\external;
...
use external_value;
use context;
use qtype_pythonrunner_sandbox;
use qtype_pythonrunner_exception;
...
class run_in_sandbox extends external_api {
     ...
     * @throws qtype_pythonrunner_exception
     */
    public static function execute($contextid, $sourcecode, $language='python3',
            $stdin='', $files='', $params='') {
        global $USER;
        // First, see if the web service is enabled.
        if (!get_config('qtype_pythonrunner', 'wsenabled')) {
            throw new qtype_pythonrunner_exception(pythonrunner_str('wsdisabled'));
        }
        ...
        if (!has_capability('qtype/pythonrunner:sandboxwsaccess', $context, $USER->id)) {
            throw new qtype_pythonrunner_exception(pythonrunner_str('wsnoaccess'));
        }

        $sandbox = qtype_pythonrunner_sandbox::get_best_sandbox($language);
        if ($sandbox === null) {
            throw new qtype_pythonrunner_exception("Language {$language} is not available on this system");
        }

        if (get_config('qtype_pythonrunner', 'wsloggingenabled')) {
            ...
            $maxhourlyrate = intval(get_config('qtype_pythonrunner', 'wsmaxhourlyrate'));
            if ($maxhourlyrate > 0) {
                $hour_ago = strtotime('-1 hour');
                $select = "userid = :userid AND eventname = :eventname AND timecreated > :since";
                $log_params = array('userid' => $USER->id, 'since' => $hour_ago,
                    'eventname' => '\qtype_pythonrunner\event\sandbox_webservice_exec');
                $currentrate = $reader->get_events_select_count($select, $log_params);
                if ($currentrate >= $maxhourlyrate) {
                    throw new qtype_pythonrunner_exception(sqlrunner_str('wssubmissionrateexceeded'));
                }
            }

            $event = \qtype_pythonrunner\event\sandbox_webservice_exec::create([
        ...
        try {
            ...
            $maxcputime = intval(get_config('qtype_pythonrunner', 'wsmaxcputime'));  // Limit CPU time through this service.
            if (isset($paramsarray['cputime'])) {
                $paramsarray['cputime'] = min($paramsarray['cputime'], $maxcputime);
            } else {
                $paramsarray['cputime'] = $maxcputime;
            }
            $jobehostws = trim(get_config('qtype_pythonrunner', 'wsjobeserver'));
            if ($jobehostws !== '') {
                $paramsarray['jobeserver'] = $jobehostws;
            }
            $runresult = $sandbox->execute($sourcecode, $language, $stdin, $filesarray, $paramsarray);
        } catch (Exception $ex) {
            throw new qtype_pythonrunner_exception("Attempt to run job failed with error {$ex->getMessage()}");
        }
