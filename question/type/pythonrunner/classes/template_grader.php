...
/** The qtype_pythonrunner_per_test_template_grader class.
...
class qtype_pythonrunner_template_grader extends qtype_sqlrunner_template_grader {
    ...
    protected function grade_known_good(&$output, &$testcase) {
        ...
        if ($result === null || !isset($result->fraction) || !is_numeric($result->fraction)) {
            ...
            $errormessage = pythonrunner_str($errorcode, array('output' => $output));
            $testresultobj = new qtype_sqlrunner_test_result($testcase, false, 0.0, $errormessage);
        } else {
            ...
            $testresultobj = new qtype_sqlrunner_test_result($testcase, $iscorrect, $awarded, $got);
