...
namespace qtype_pythonrunner\form;
...
class dataset_manager_form extends \moodleform {
    ...
    public function validation($data, $files){
        ...
        foreach ($data['dataset'] as $key => $draftid){
            ...
            if (!empty($duplicated_tables)){
                $errors[$tables_field] = pythonrunner_str('error_duplicated_tables', implode(',', $duplicated_tables))."</br>";
            }
            if (!empty($missing_file)){
                $errors[$tables_field] .= pythonrunner_str('error_missed_file_tables', implode(',', $missing_file))."</br>";
            }
            if (!empty($files_tables)){
                $errors[$tables_field] .= pythonrunner_str('error_missed_inputted_tables', implode(',', array_keys($files_tables)))."</br>";
            }
            if (!empty($existed_tables)){
                $errors[$tables_field] = pythonrunner_str('error_existed_table', implode(',', $existed_tables))."</br>";
            }
    ...
    protected function _get_multiple_fields($mform, &$repeatedoptions){
        $groupelements = array();
        $groupelements[] = $mform->createElement('filemanager', 'dataset', pythonrunner_str('form_dataset_name'), null, $this->_fileoptions);
        $groupelements[] = $mform->createElement('text', 'dataset_tables', pythonrunner_str('form_dataset_tables_name'));
