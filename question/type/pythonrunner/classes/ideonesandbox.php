...
class qtype_pythonrunner_ideonesandbox extends qtype_pythonrunner_sandbox {
    ...
    public function __construct($user=null, $pass=null) {
        if ($user == null) {
            $user = get_config('qtype_pythonrunner', 'ideone_user');
        }

        if ($pass == null) {
            $pass = get_config('qtype_pythonrunner', 'ideone_password');
        }

        qtype_pythonrunner_sandbox::__construct($user, $pass);
    ...
    /** Main interface function for use by pythonrunner but not part of ideone API.
    ...
    public function execute($sourcecode, $language, $input, $files=null, $params=null) {
        ...
        if (!in_array($language, $this->get_languages()->languages)) {
            throw new qtype_pythonrunner_exception('Executing an unsupported language in sandbox');
        }
        ...
        if ($error != self::OK) {
            ...
            if ($count >= self::MAX_NUM_POLLS) {
                throw new qtype_pythonrunner_exception("Timed out waiting for sandbox");
            }
