...
global $CFG;
require_once $CFG->dirroot . '/question/type/pythonrunner/classes/twigmacros.php';

// Class that provides a singleton instance of the twig environment.
class qtype_pythonrunner_twig extends qtype_sqlrunner_twig{
    ...
    protected static function get_twig_environment($isstrict=false, $isdebug=false) {
        if (self::$twigenvironments[$isstrict] === null) {
            // On the first call, build the required environment.
            $macros = qtype_pythonrunner_twigmacros::macros();
            ...
            $newrandom = new \Twig\TwigFunction('random', 'qtype_pythonrunner_twig_random',
                array('needs_environment' => true));
            $setrandomseed = new \Twig\TwigFunction('set_random_seed', 'qtype_pythonrunner_set_random_seed',
                array('needs_environment' => true));
            $twig->addFunction($newrandom);
            $twig->addFunction($setrandomseed);
            $shuffle = new \Twig\TwigFilter('shuffle', 'qtype_pythonrunner_twig_shuffle',
                array('needs_environment' => true));
            ...
            $escaperextension->setEscaper('py', 'qtype_sqlrunner_escapers::python');
            $escaperextension->setEscaper('python', 'qtype_sqlrunner_escapers::python');
            $escaperextension->setEscaper('c',  'qtype_sqlrunner_escapers::java');
            $escaperextension->setEscaper('java', 'qtype_sqlrunner_escapers::java');
            $escaperextension->setEscaper('ml', 'qtype_sqlrunner_escapers::matlab');
            $escaperextension->setEscaper('matlab', 'qtype_sqlrunner_escapers::matlab');
        }
    ...
    public static function render($s, $student, $parameters=array(), $isstrict=false) {
        $twig = qtype_pythonrunner_twig::get_twig_environment($isstrict);
        $parameters['STUDENT'] = new qtype_sqlrunner_student($student);
    ...
    protected static function get_policy() {
        ...
        $methods = array(
            'stdClass' => array(),
            'qtype_sqlrunner_student' => '*'
        );
        $properties = array(
            'stdClass' => '*',
            'qtype_sqlrunner_student' => '*',
            'qtype_pythonrunner_question' => '*'
        );
        $policy = new qtype_sqlrunner_twig_security_policy($tags, $filters, $methods, $properties, $functions);
        return $policy;
    }
}
 ...
 */
function qtype_pythonrunner_twig_random(Twig\Environment $env, $values = null, $max = null)
{
    if (null === $values) {
 ...
 */
function qtype_pythonrunner_set_random_seed(Twig\Environment $env, $seed)
{
    mt_srand($seed);
    return '';
}
 ...
 */
function qtype_pythonrunner_twig_shuffle(Twig\Environment $env, $array) {
    shuffle($array);
    return $array;
}
