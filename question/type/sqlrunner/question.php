 ...
 */
class qtype_sqlrunner_question extends question_graded_automatically {
...
//            $this->student = new qtype_sqlrunner_student($DB->get_record('user', array('id' => $userid)));
//            $step->set_qt_var('_STUDENT', serialize($this->student));
        }
//        else {  // Validation, so just use the global $USER as student.
//            $this->student = new qtype_sqlrunner_student($USER);
//        }
    ...
    public function evaluate_question_for_display($seed, $step) {
        ...
        try {
            ...
            $erroroninit = sqlrunner_str('erroroninit', array('error' => $error));
...
//        }
    ...
    protected function evaluate_template_params_on_jobe($templateparams, $lang, $seed) {
        ...
        if ($run->error === qtype_sqlrunner_sandbox::SERVER_OVERLOAD) {
        ...
        } else if ($run->error !== qtype_sqlrunner_sandbox::OK) {
            return qtype_sqlrunner_sandbox::error_string($run);
        } else if ($run->result != qtype_sqlrunner_sandbox::RESULT_SUCCESS) {
            return qtype_sqlrunner_sandbox::result_string($run->result) . "\n" . $run->cmpinfo . $run->output . $run->stderr;
        } else {
            return $run->output;
        }
    }
    ...
    protected function twig_render_with_seed($text, $seed) {
        mt_srand($seed);
        return qtype_sqlrunner_twig::render($text, $this->student);
    }
     ...
     * that caches test results returned by the call to @see static::grade_response().
     *
    ...
    public function get_expected_data() {
        $expecteddata = array('answer' => PARAM_RAW, 'language' => PARAM_NOTAGS);
...
//                return sqlrunner_str('badfiles', $badfilelist);
//            }
//        } else {
//            $attachcount = 0;
//        }
...
//            return sqlrunner_str('insufficientattachments', $this->attachmentsrequired);
//        }
...
//                return sqlrunner_str('answerrequired');
//            } else if (strlen($response['answer']) < constants::FUNC_MIN_LENGTH) {
//                return sqlrunner_str('answertooshort', constants::FUNC_MIN_LENGTH);
//            }
//        }
        return '';  // All good.
    }
    ...
    protected function is_valid_filename($filename, $regex, $supportfiles) {
    ...
    public function get_validation_error(array $response) {
        ...
        if ($error) {
            ...
            return sqlrunner_str('unknownerror');
        }
    }
    ...
    public function is_same_response(array $prevresponse, array $newresponse) {
        $sameanswer = question_utils::arrays_same_at_key_missing_is_blank($prevresponse, $newresponse, 'answer')
    ...
    public function get_correct_answer() {
        ...
        if (!isset($this->answer)) {
            ...
            if (!empty($params->answer_language)) {
                ...
                list($langs, $defaultlang) = qtype_sqlrunner_util::extract_languages($this->acelang);
     ...
     * qtype_sqlrunner_testing_outcome object to be cached. The invalid
    ...
    public function grade_response(array $response, bool $isprecheck = false, bool $is_edit = false){
        ...
        if (!empty($response['_testoutcome'])){
            ...
            if ($testoutcome instanceof qtype_sqlrunner_testing_outcome  // Ignore legacy-format outcomes.
        ...
        if ($gradingreqd){
            ...
            $testoutcome = $this->run($response, $isprecheck, $language);
        }

        return $this->_get_grade_response_data($testoutcome, $testoutcomeserial, $is_edit);
    }

    public function run($response, $isprecheck, $language, $runner = null){
        ...
        return $runner->run_tests($this, $code, $attachments, $testcases, $isprecheck, $language);
    }

    protected function _get_grade_response_data($testoutcome, $testoutcomeserial = null, $is_edit = false){
        if (is_null($testoutcomeserial)){
            $testoutcomeserial = serialize($testoutcome);
        }
        $datatocache = array('_testoutcome' => $testoutcomeserial);
        $testresult = reset($testoutcome->testresults);
        // save student output to another step, because test_outcome is serialised and big strings are snip.
        // also sql_result can be encoded and contains non-ASCII symbols
        if (!empty($testresult)){
            $datatocache['_runneroutput'] = $testresult->got;
        } elseif (!empty($testoutcome->errormessage)) {
            $datatocache['_runnererror'] = $testoutcome->errormessage;
        }

        if ($testoutcome->run_failed()){
            return array(0, question_state::$invalid, $datatocache);
        } elseif ($testoutcome->all_correct()) {
            return array(1, question_state::$gradedright, $datatocache);
        } elseif ($this->grader === 'TemplateGrader' && $this->iscombinatortemplate) {
            return array(0, question_state::$gradedwrong, $datatocache);
        } else {
            $mark = $testoutcome->mark_as_fraction();
            if ($mark === 1){
                return array(1, question_state::$gradedright, $datatocache);
            } else {
                return array(0, question_state::$gradedwrong, $datatocache);
            ...
            return array($mark, question_state::$gradedpartial, $datatocache);
        }
    }

    // Return a map from filename to file contents for all the attached files
    // in the given response.
    protected function get_attached_files($response) {
        $attachments = array();
    ...
    protected static function step_info($response) {
        ...
        $stepinfo->sqlrunnerversion = get_config('qtype_sqlrunner')->version;
        return $stepinfo;
    }
    ...
    public function result_columns() {
        if (isset($this->resultcolumns) && $this->resultcolumns) {
            ...
            $resultcolumns = array(
                array(sqlrunner_str('testcolhdr'), 'testcode'),
                array(sqlrunner_str('inputcolhdr'), 'stdin'),
                array(sqlrunner_str('expectedcolhdr'), 'expected'),
                array(sqlrunner_str('gotcolhdr'), 'got'),
            );
        }
    ...
    public function twig_expand($text, $context=array()) {
        ...
        return trim(qtype_sqlrunner_twig::render($text, null, $context));
    }
    ...
    public function filter_testcases($isprecheckrun, $prechecksetting) {
        if (!$isprecheckrun) {
    ...
    protected function empty_testcase() {
        return (object) array(
            'testtype' => 0,
            'testcode' => '',
            'stdin'    => '',
    ...
    public function get_sandbox($additional_params = []) {
        ...
        if ($sandbox === null) {   // No sandbox specified. Use best we can find.
            $sandboxinstance = qtype_sqlrunner_sandbox::get_best_sandbox($this->language);
            if ($sandboxinstance === null) {
                throw new qtype_sqlrunner_exception("Language {$this->language} is not available on this system");
            }
        } else {
            $sandboxinstance = qtype_sqlrunner_sandbox::get_instance($sandbox);
            if ($sandboxinstance === null) {
                throw new qtype_sqlrunner_exception("Question is configured to use a non-existent or disabled sandbox ($sandbox)");
            }
        }
    ...
    public function get_grader() {
        ...
        $graders = qtype_sqlrunner_grader::available_graders();
        $graderclass = $graders[$grader];
        return new $graderclass();
    }

    public static function get_empty_question($code = '', $expected = '', $rownum = 0){
        $question = new static();
        $question->prototype = 1;
        $question->grader = constants::DEFAULT_GRADER;
        $question->language = 'sql';
        $question->iscombinatortemplate = 1;
        $question->answer = $code;
        $question->testsplitterre = '|#<ab@17943918#@>#\n|ms';
        $question->testcases = [
            (object)[
                'questionid'     => 0,
                'testtype'       => 0,
                'rownum'         => $rownum,
                'testcode'       => qtype_sqlrunner_util::filter_crs($question->answer),
                'stdin'          => '',
                'expected'       => $expected,
                'extra'          => '',
                'useasexample'   => false,
    ...
    public function get_files() {
        if ($this->prototypetype != 0) { // Is this a prototype question?
            ...
            $this->get_prototype();
            $files = static::get_support_files($this->prototype);
        }
        $files = array_merge($files, static::get_support_files($this));  // Add in files for this question.
        return $files;
    }
    ...
    public function get_prototype() {
        ...
        if ($this->prototypetype == 0) {
            $context = qtype_sqlrunner::question_context($this);
            $this->prototype = qtype_sqlrunner::get_prototype($this->sqlrunnertype, $context);
        } else {
            $this->prototype = null;
        }
    }
    ...
    protected static function get_support_files($question) {
        ...
        if (isset($question->contextid)) {
            ...
            $context = qtype_sqlrunner::question_context($question);
        ...
        if (isset($question->supportfilemanagerdraftid)) {
            ...
            $files = $fs->get_area_files($contextid, 'qtype_sqlrunner', 'datafile', $question->id);
        }
