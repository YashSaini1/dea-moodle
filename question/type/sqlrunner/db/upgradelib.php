 ...
 */
function qtype_sqlrunner_update_question_types() {
    $systemcontext = context_system::instance();
    $systemcontextid = $systemcontext->id;

    qtype_sqlrunner_delete_existing_prototypes($systemcontextid);
    if (function_exists('question_get_top_category')) { // Moodle version >= 3.5.
        $parentid = qtype_sqlrunner_get_top_id($systemcontextid);
    } else {
        $parentid = 0;
    }
    $prototypescategory = qtype_sqlrunner_find_or_make_prototype_category($systemcontextid, $parentid);
    qtype_sqlrunner_load_new_prototypes($systemcontextid, $prototypescategory);
    return true;
}
...
function qtype_sqlrunner_delete_existing_prototypes($systemcontextid) {
    global $DB;
    ...
    foreach ($prototypes as $question) {
        $DB->delete_records('question_sqlrunner_options', array('questionid' => $question->id));
...
function qtype_sqlrunner_get_top_id($systemcontextid) {
    global $DB;
    $topid = 0;
    ...
    if ($topid === 0 && $prototypecategoryid === 0) {
        ...
        qtype_sqlrunner_make_cr_prototypes_top($prototypecat);
    } else if ($topid !== 0 && $prototypecategoryid !== 0) {
...
function qtype_sqlrunner_find_or_make_prototype_category($systemcontextid, $parentid) {
    global $DB;
    ...
    if (!$category) {
        $category = array(
            ...
            'info'       => 'Category for sqlrunner question built-in prototypes. FOR SYSTEM USE ONLY.',
            'parent'     => $parentid,
            'sortorder'  => 999,
...
function qtype_sqlrunner_make_cr_prototypes_top($category) {
    global $DB;
...
function qtype_sqlrunner_load_new_prototypes($systemcontextid, $prototypecategory) {
    $dbfiles = scandir(__DIR__);
    foreach ($dbfiles as $file) {
        if (strpos(strrev($file), strrev('_PROTOTYPES.xml')) === 0) {
            $filename = __DIR__ . '/' . $file;
            qtype_sqlrunner_load_questions($prototypecategory, $filename, $systemcontextid);
        }
    }
}
 ...
 */
function qtype_sqlrunner_load_questions($category, $importfilename, $contextid) {
    global $COURSE;
