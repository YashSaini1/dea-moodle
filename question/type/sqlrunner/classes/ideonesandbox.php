...
class qtype_sqlrunner_ideonesandbox extends qtype_sqlrunner_sandbox {
    ...
    public function __construct($user=null, $pass=null) {
        if ($user == null) {
            $user = get_config('qtype_sqlrunner', 'ideone_user');
        }

        if ($pass == null) {
            $pass = get_config('qtype_sqlrunner', 'ideone_password');
        }
    ...
    /** Main interface function for use by sqlrunner but not part of ideone API.
    ...
    public function execute($sourcecode, $language, $input, $files=null, $params=null) {
        ...
        if (!in_array($language, $this->get_languages()->languages)) {
            throw new qtype_sqlrunner_exception('Executing an unsupported language in sandbox');
        }
        ...
        if ($error != self::OK) {
            ...
            if ($count >= self::MAX_NUM_POLLS) {
                throw new qtype_sqlrunner_exception("Timed out waiting for sandbox");
            }
