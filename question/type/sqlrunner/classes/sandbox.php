...
abstract class qtype_sqlrunner_sandbox {
...
//            require_once("$filename");
    ...
    public static function get_best_sandbox($language, $forcelanguagecheck=false) {
        $sandboxes = static::enabled_sandboxes();
        if (count($sandboxes) == 0) {
            throw new qtype_sqlrunner_exception('No sandboxes available for running code!');
        }
        foreach ($sandboxes as $extname => $classname) {
            $sb = static::get_instance($extname);
            if ($sb) {
                ...
                if ($langs->error == $sb::OK) {
                    ...
                    throw new qtype_sqlrunner_exception('sandboxerror',
     ...
     * leading qtype_sqlrunner and all underscores removed.
     * @return array
     */
    public static function available_sandboxes() {
        return array('jobesandbox'      => 'qtype_sqlrunner_jobesandbox',
                     'ideonesandbox'    => 'qtype_sqlrunner_ideonesandbox',
                     'sqlsandbox'    => 'qtype_sqlrunner_sqlsandbox',
        );
    }
    ...
    public static function enabled_sandboxes() {
        $available = static::available_sandboxes();
        $enabled = array();
        foreach ($available as $extname => $classname) {
            if (get_config(static::get_plugin_name(), $extname . '_enabled')) {
    ...
    public static function get_filename($extsandboxname) {
        $boxes = static::available_sandboxes();
        $classname = $boxes[$extsandboxname];
        return str_replace('_', '', str_replace(static::get_plugin_name().'_', '', $classname)) . '.php';
    }

    /**
     *
     * @param object $runresult  (an object returned by a call to sandbox::execute).
     * @return string a description of the particular runresult
     * @throws coding_exception
     */
    public static function error_string($runresult) {
        $errorstrings = array(
            static::OK              => 'errorstring-ok',
            static::AUTH_ERROR      => 'errorstring-autherror',
            static::PASTE_NOT_FOUND => 'errorstring-pastenotfound',
            static::WRONG_LANG_ID   => 'errorstring-wronglangid',
            static::ACCESS_DENIED   => 'errorstring-accessdenied',
            static::SUBMISSION_LIMIT_EXCEEDED  => 'errorstring-submissionlimitexceeded',
            static::CREATE_SUBMISSION_FAILED  => 'errorstring-submissionfailed',
            static::UNKNOWN_SERVER_ERROR  => 'errorstring-unknown',
            static::JOBE_400_ERROR  => 'errorstring-jobe400',
            static::SERVER_OVERLOAD => 'errorstring-overload',
        );
        $errorcode = $runresult->error;
        if (!isset($errorstrings[$errorcode])) {
            throw new coding_exception("Bad call to sandbox.errorString");
        }
        if ($errorcode != static::JOBE_400_ERROR || !isset($runresult->stderr)) {
            return !empty($runresult->stderr) ? $runresult->stderr : sqlrunner_str($errorstrings[$errorcode]);
        } else {
            // Special case for JOBE_400 error. Include HTTP error message in
            // the returned error.
            $message = sqlrunner_str('errorstring-jobe400');
    ...
    public static function result_string($resultcode) {
        $resultstrings = array(
            static::RESULT_NO_RUN               => 'resultstring-norun',
            static::RESULT_COMPILATION_ERROR    => 'resultstring-compilationerror',
            static::RESULT_RUNTIME_ERROR        => 'resultstring-runtimeerror',
            static::RESULT_TIME_LIMIT           => 'resultstring-timelimit',
            static::RESULT_SUCCESS              => 'resultstring-success',
            static::RESULT_MEMORY_LIMIT         => 'resultstring-memorylimit',
            static::RESULT_ILLEGAL_SYSCALL      => 'resultstring-illegalsyscall',
            static::RESULT_INTERNAL_ERR         => 'resultstring-internalerror',
            static::RESULT_OUTPUT_LIMIT         => 'resultstring-outputlimit',
            static::RESULT_ABNORMAL_TERMINATION => 'resultstring-abnormaltermination',
            static::RESULT_SERVER_OVERLOAD      => 'resultstring-sandboxoverload',
        );
        if (!isset($resultstrings[$resultcode])) {
            throw new coding_exception("Bad call to sandbox.resultString");
        }
        return get_string($resultstrings[$resultcode], static::get_plugin_name());
    }
     ...
     * @return object with at least an attribute 'error'. This is one of the
    ...
    public function test_function() {
        if ($this->authenticationerror) {
            return (object) array('error' => static::AUTH_ERROR);
        } else {
            return (object) array(
                'error' => static::OK,
