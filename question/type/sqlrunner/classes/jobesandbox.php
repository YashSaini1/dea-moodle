...
class qtype_sqlrunner_jobesandbox extends qtype_sqlrunner_sandbox {
     ...
     * header X-sqlrunner-Job-Id: to every API call we make.
     *
     ...
     */
    protected $currentjobid = null;

    protected $languages = null;   // Languages supported by this sandbox.
    protected $httpcode = null;    // HTTP response code.
    protected $response = null;    // Response from HTTP request to server.
    ...
    public function __construct() {
        ...
        if ($CFG->prefix == 'bht_') {
            ...
            $this->jobeserver = get_config(static::get_plugin_name(), 'jobe_host');
        }

        $this->apikey = get_config(static::get_plugin_name(), 'jobe_apikey');
        $this->languages = null;
    }
    ...
    public function execute($sourcecode, $language, $input, $files=null, $params=null) {
        ...
        return $this->_get_execute_result($httpcode);
    }
    ...
    protected function _get_execute_result($httpcode){
        ...
        if ($httpcode != 200   // We don't deal with Jobe servers that return 202!
            ...
            $errorcode = $httpcode == 200 ? static::UNKNOWN_SERVER_ERROR : $this->get_error_code($httpcode);
        ...
        if ($this->response->outcome == static::RESULT_SERVER_OVERLOAD){
            $runresult['error'] = static::SERVER_OVERLOAD;
            return (object)$runresult;
        }
        ...
        if (trim($stderr) !== ''){
            $this->response->outcome = static::RESULT_RUNTIME_ERROR;
        }

        $output = $this->format_output($this->response->stdout);
        if (empty($output)){
            $output = \qtype_sqlrunner\constants::EMPTY_SET;
        }
        $this->currentjobid = null;
    ...
    protected function format_output($output){
        return trim($this->filter_file_path($output));
    }
    ...
    protected function get_main_class($prog) {
    ...
    protected function put_file($contents) {
        $id = md5($contents);
     ...
     */
    protected function get_jobe_connection_info($resource) {
    ...
    protected function submit($job) {
        list($returncode, $response) = $this->http_request('runs', static::HTTP_POST, $job);
     ...
     * resource using the given method (static::HTTP_GET or static::HTTP_POST).
     ...
     */
    protected function http_request($resource, $method, $body = null){
        list($url, $headers) = $this->get_jobe_connection_info($resource);

        $curl = new curl();
        $curl->setHeader($headers);

        if ($method === static::HTTP_GET) {
            if (!empty($body)) {
                throw new coding_exception("Illegal HTTP GET: non-empty body");
            }
            $response = $curl->get($url);
        } else if ($method === static::HTTP_POST) {
            if (empty($body)) {
    ...
    // with filename.
    protected function filter_file_path($s) {
