...
/** Defines a subclass of the normal sqlrunner testing_outcome for use when
...
class qtype_sqlrunner_combinator_grader_outcome extends qtype_sqlrunner_testing_outcome {
     ...
     * @param qtype_sqlrunner_question $q The question being rendered (ignored)
     * @return A table of test results. See the parent class for details.
     */
    public function get_test_results(qtype_sqlrunner_question $q) {
    ...
    private function validate_table_formats() {
        if ($this->columnformats && $this->testresults) {
            ...
            if (count($this->columnformats) !== $numcols) {
                $error = sqlrunner_str('wrongnumberofformats', array('expected' => $numcols, 'got' => count($this->columnformats)));
                $this->set_status(self::STATUS_BAD_COMBINATOR, $error);
            } else {
                foreach ($this->columnformats as $format) {
                    if ($format !== '%s' && $format !== '%h') {
                        $error = sqlrunner_str('illegalformat', array('format' => $format));
                        $this->set_status(self::STATUS_BAD_COMBINATOR, $error);
                        break;
                    }
                }
