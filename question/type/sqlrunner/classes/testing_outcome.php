...
class qtype_sqlrunner_testing_outcome {
    ...
    public $grader;                  // An associative array of sandbox info, e.g. Jobe server name.

    public function __construct($maxpossmark, $numtestsexpected, $isprecheck, $grader = null) {
    ...
    public function validation_error_message() {
        ...
        if ($this->iscombinatorgrader()){
            // Combinator grader results table can't be used.
            return sqlrunner_str('failedtesting').html_writer::empty_tag('br').pythonrunner_str('howtogetmore');
        }
    ...
    protected function print_admin_result_table(){
        ...
        foreach ($this->testresults as $i => $testresult){
            if ($testresult->iscorrect) continue;
            ...
            if (isset($testresult->expected) && isset($testresult->got)){
                ...
                $o .= '<div class="failrow_'.$rownum.'">
                        ...
                        </div>'.render_submission_table($testresult->got);
//                    $failures->data[] = array(
////                        html_writer::link('#id_testcode_' . $rownum,
////                               sqlrunner_str('testcase', $rownum + 1) .
////                                    html_writer::empty_tag('br') . s($testresult->testcode)),
////                        '',
//                        html_writer::link('#id_expected_' . $rownum, html_writer::tag('pre', s($testresult->expected), array('id' => 'id_fail_expected_' . $rownum, 'style' => 'display:none'))),
//                        html_writer::tag('pre', s($testresult->got), array('id' => 'id_got_' . $rownum, 'style' => 'display:none')) .
//                            html_writer::tag('button', $button_name, array(
//                                'type' => 'button',  // To suppress form submission.
//                                'class' => 'replaceexpectedwithgot')),
//                    );
//                    $failures->rowclasses[] = 'sqlrunner-failed-test failrow_' . $rownum;
            }
        }
        if (empty($o)){
            return $o;
     ...
     *
     * @global type $COURSE
     * @param qtype_sqlrunner $question
    ...
    public static function can_view_hidden() {
        ...
        if ($COURSE && $coursecontext = context_course::instance($COURSE->id)) {
            $canviewhidden = has_capability('qtype/sqlrunner:viewhiddentestcases', $coursecontext);
        } else {
            $canviewhidden = false;
        }
