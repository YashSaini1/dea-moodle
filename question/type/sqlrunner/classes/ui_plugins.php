...
class qtype_sqlrunner_ui_plugins {
    ...
    public function __construct() {
        ...
        $files = scandir($CFG->dirroot . '/question/type/sqlrunner/amd/src');
        $this->plugins = array();
        foreach ($files as $file) {
            if (substr($file, 0, 3) === 'ui_' && substr($file, -3) === '.js') {
                $uiname = substr($file, 3, -3);
                $parameters = new qtype_sqlrunner_ui_parameters($uiname);
                $plugin = new qtype_sqlrunner_ui_plugin($uiname, $parameters);
                $this->plugins[$uiname] = $plugin;
            }
        }

        // Add a dummy 'None' uiplugin.
        $emptyparameters = new qtype_sqlrunner_ui_parameters('none');
        $this->plugins['none'] = new qtype_sqlrunner_ui_plugin(null, $emptyparameters);
    }
    ...
    /**
     * Get the qtype_sqlrunner_ui_plugin object for the given plugin name.
     * @param string $uiname
     * @return qtype_sqlrunner_ui_plugin the specs for the given plugin.
     */
...
class qtype_sqlrunner_ui_plugin {

    /**
     *
     * @param string-or-null $filename the ui plugins filename or null for 'None'
     * @param qtype_sqlrunner_ui_parameters $parameters the set of parameters available.
     */
    public function __construct($uiname, qtype_sqlrunner_ui_parameters $parameters) {
        if ($uiname !== null) {
            $this->uiname = $uiname;
        } else {
    ...
    /**
     *
     * @return qtype_sqlrunner_ui_parameters the parameters for this plugin.
     */
