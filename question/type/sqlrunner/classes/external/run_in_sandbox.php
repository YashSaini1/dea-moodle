...
/*
 * qtype_sqlrunner external file. Allows webservice access by authenticated
...
namespace qtype_sqlrunner\external;
...
use external_value;
use context;
use qtype_sqlrunner_sandbox;
use qtype_sqlrunner_exception;
...
class run_in_sandbox extends external_api {
     ...
     * @throws qtype_sqlrunner_exception
     */
    public static function execute($contextid, $sourcecode, $language='python3',
            $stdin='', $files='', $params='') {
        global $USER;
        // First, see if the web service is enabled.
        if (!get_config('qtype_sqlrunner', 'wsenabled')) {
            throw new qtype_sqlrunner_exception(sqlrunner_str('wsdisabled'));
        }
        ...
        if (!has_capability('qtype/sqlrunner:sandboxwsaccess', $context, $USER->id)) {
            throw new qtype_sqlrunner_exception(sqlrunner_str('wsnoaccess'));
        }

        $sandbox = qtype_sqlrunner_sandbox::get_best_sandbox($language);
        if ($sandbox === null) {
            throw new qtype_sqlrunner_exception("Language {$language} is not available on this system");
        }

        if (get_config('qtype_sqlrunner', 'wsloggingenabled')) {
            ...
            $maxhourlyrate = intval(get_config('qtype_sqlrunner', 'wsmaxhourlyrate'));
            if ($maxhourlyrate > 0) {
                $hour_ago = strtotime('-1 hour');
                $select = "userid = :userid AND eventname = :eventname AND timecreated > :since";
                $log_params = array('userid' => $USER->id, 'since' => $hour_ago,
                    'eventname' => '\qtype_sqlrunner\event\sandbox_webservice_exec');
                $currentrate = $reader->get_events_select_count($select, $log_params);
                if ($currentrate >= $maxhourlyrate) {
                    throw new qtype_sqlrunner_exception(sqlrunner_str('wssubmissionrateexceeded'));
                }
            }

            $event = \qtype_sqlrunner\event\sandbox_webservice_exec::create([
        ...
        try {
            ...
            $maxcputime = intval(get_config('qtype_sqlrunner', 'wsmaxcputime'));  // Limit CPU time through this service.
            if (isset($paramsarray['cputime'])) {
                $paramsarray['cputime'] = min($paramsarray['cputime'], $maxcputime);
            } else {
                $paramsarray['cputime'] = $maxcputime;
            }
            $jobehostws = trim(get_config('qtype_sqlrunner', 'wsjobeserver'));
            if ($jobehostws !== '') {
                $paramsarray['jobeserver'] = $jobehostws;
            }
            $runresult = $sandbox->execute($sourcecode, $language, $stdin, $filesarray, $paramsarray);
        } catch (Exception $ex) {
            throw new qtype_sqlrunner_exception("Attempt to run job failed with error {$ex->message}");
        }
