...
/**
 * Utility routines for qtype_sqlrunner
 */
...
class qtype_sqlrunner_util {
    ...
    public static function load_uiplugin_js($question, $textareaid) {
        ...
        $uiplugin = empty($question->uiplugin) ? 'ace' : strtolower($question->uiplugin);
        if ($uiplugin !== '' && $uiplugin !== 'none') {
            $params = array($uiplugin, $textareaid);  // Params to plugin's init function.
            if (strpos($uiplugin, 'ace') !== false) {
                self::load_ace();
            }
            $PAGE->requires->js_call_amd('qtype_sqlrunner/userinterfacewrapper', 'newUiWrapper', $params);
        }
    }
    ...
    public static function load_ace() {
        global $PAGE;
        if ($PAGE->requires->should_create_one_time_item_now('qtype_sqlrunner-ace-scripts')) {
            $plugindirrel = '/question/type/sqlrunner';
    ...
    public static function template_params($jsonparams) {
        if (empty($jsonparams)) {
            ...
            if ($params === null) {
                throw new qtype_sqlrunner_bad_json_exception("$jsonparams");
            }
            return $params;
        }
    }
    ...
    public static function filter_crs($s) {
        $s = str_replace("\r", "", $s);
