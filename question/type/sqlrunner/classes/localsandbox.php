...
/** A LocalSandbox is a subclass of the base qtype_sqlrunner_sandbox class,
...
abstract class qtype_sqlrunner_localsandbox extends qtype_sqlrunner_sandbox {
    ...
    public function __construct($user=null, $pass=null) {
        qtype_sqlrunner_sandbox::__construct($user, $pass);
    }
    ...
    public function execute($sourcecode, $language, $input, $files=null, $params=null) {
        ...
        if (!in_array($language, $this->get_languages()->languages)) {
            return (object) array('error' => static::WRONG_LANG_ID);  // Should be impossible.
        }
        ...
        $error = static::OK; // Start by being optimistic.
        ...
        if (empty($this->source) || $this->source !== $sourcecode) {
            ...
            $this->save_source();
            $error = $this->compile();
            if ($error === static::OK && !empty($this->cmpinfo)) {
                $this->result = static::RESULT_COMPILATION_ERROR;
            }
        }
    ...
    protected function make_directory() {
        $this->workdir = tempnam("/tmp", "sqlrunner_");
     ...
     * naming it static::SOURCE_FILE_NAME. That name is recorded in
     * $this->sourcefilename.
     */
    protected function save_source() {
        assert(!empty($this->workdir));
        chdir($this->workdir);
        $handle = fopen(static::SOURCE_FILE_NAME, "w");
        fwrite($handle, $this->source);
        fclose($handle);
        $this->sourcefilename = static::SOURCE_FILE_NAME;
    }
    ...
    public function close() {
        static::del_tree($this->workdir);
    }


    // Delete a given directory tree.
    protected static function del_tree($dir) {
     ...
     * run_in_sandbox.
     * @return qtype_sqlrunner_sandbox::ok if run succeeds in the sense
     * of nothing going terribly wrong or qtype_sqlrunner_sandbox::UNKNOWN_SERVER_ERROR
     * otherwise.
     */
