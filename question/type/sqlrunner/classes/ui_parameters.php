...
class qtype_sqlrunner_ui_parameter {
...
/**
 * A class representing a set of qtype_sqlrunner_ui_parameter objects for
 * a particular ui plugin.
 */
class qtype_sqlrunner_ui_parameters {
    ...
    public function __construct(string $uiname) {
        global $CFG;
        $filename = $CFG->dirroot . "/question/type/sqlrunner/amd/src/ui_{$uiname}.json";
        $this->uiname = $uiname;
        $this->params = array();
        if (file_exists($filename)) {
            ...
            foreach ((array) $spec->parameters as $name => $obj) {
                ...
                $this->params[$name] = new qtype_sqlrunner_ui_parameter($name, $obj->type, $obj->default, $required);
            }
        }
    }
            ...
            foreach ($newvalues as $key => $value) {
                ...
                if ($matchingkey === null) {
                    if ($ignorebad) {
                        ...
                        throw new qtype_sqlrunner_exception("Unexpected key value ($key) when merging json for ui {$this->uiname}");
                    }
                }
     ...
     */
    protected function find_key(string $paramname) {
    ...
    public function table() {
        $table = array();
        foreach ($this->params as $param) {
            $descr = sqlrunner_str("{$this->uiname}ui_{$param->name}_descr");
