 ...
 * sqlrunner questions in a given courses.
 *
...
$PAGE->set_url('/question/type/sqlrunner/findduplicatesindex.php');
...
foreach ($questionsbycontext as $contextid => $numsqlrunnerquestions) {
    ...
    if (has_capability('moodle/question:editall', $context)) {
        $availablequestionsbycontext[$contextid] = $numsqlrunnerquestions;
    }
}
...
if (count($availablequestionsbycontext) == 0) {
    ...
    foreach ($availablequestionsbycontext as $contextid => $numsqlrunnerquestions) {
        $context = context::instance_by_id($contextid);
        $name = $context->get_context_name(true, true);
        if (strpos($name, 'Course:') === 0) {
            $class = 'bulktest sqlrunner context quiz';
            $findduplicatesurl = new moodle_url('/question/type/sqlrunner/findduplicates.php', array('contextid' => $contextid));
            $findduplicateslink = html_writer::link($findduplicatesurl,
                    'Find duplicates',
                    array('title' => 'Find all duplicates in this context',
                          'style' => $buttonstyle));
            $litext = $name . ' (' . $numsqlrunnerquestions . ') ' . $findduplicateslink;
