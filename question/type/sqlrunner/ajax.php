 ...
 * question type by looking up the prototype in the question_sqlrunner_options
...
if ($qtype) {
    $questiontype = qtype_sqlrunner::get_prototype($qtype, $coursecontext);
    ...
    if (!in_array($uiplugin, $allnames)) {
        ...
        if ($uiparams->length() === 0) {
            $uiheader = sqlrunner_str('nouiparameters', array('uiname' => $uiplugin));
        } else {
            $csv = implode(', ', $uiparams->all_names_starred());
            $uiheader = sqlrunner_str('uiparametertablehead',
    ...
    echo json_encode(array('header' => $uiheader,
        ...
        'showdetails' => sqlrunner_str('showdetails'),
        'hidedetails' => sqlrunner_str('hidedetails')));
}
die();
