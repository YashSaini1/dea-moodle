...
/**
 * Restore plugin class for sqlrunner questions.
 */
class restore_qtype_sqlrunner_plugin extends restore_qtype_plugin {
    ...
    private function add_question_options(&$paths) {
        ...
        $elename = 'sqlrunner_options';
        $elepath = $this->get_pathfor('/sqlrunner_options/sqlrunner_option');  // We used get_recommended_name() so this works.
    ...
    private function add_question_testcases(&$paths) {
        ...
        $elename = 'sqlrunner_testcases';
        $elepath = $this->get_pathfor('/sqlrunner_testcases/sqlrunner_testcase');
        $paths[] = new restore_path_element($elename, $elepath);
    }
     ...
     * backup element.
     */
    public function process_sqlrunner_testcases($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            // Insert record.
            $newitemid = $DB->insert_record("question_sqlrunner_tests", $data);
        }
     ...
     * backup element.
     */
    public function process_sqlrunner_options($data) {
        global $DB;
        ...
        if ($questioncreated) {
            ...
            // Insert the record.
            $newitemid = $DB->insert_record("question_sqlrunner_options", $data);
        }
        // Nothing to remap if the question already existed.
