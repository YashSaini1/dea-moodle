...
// A sqlrunner question consists of a specification for a piece of program
...
// sandbox.
// In a typical use case each sqlrunner question will have its
...
/**
 * qtype_sqlrunner extends the base question_type to sqlrunner-specific functionality.
 * A sqlrunner question requires an additional DB table
 ...
 */
class qtype_sqlrunner extends question_type {
    ...
    public function extra_question_fields() {
        return array('question_sqlrunner_options',
            'sqlrunnertype',
            'prototypetype',
            'allornothing',
            'penaltyregime',
...
//            'attachmentsrequired',
    ...
    private function copy_testcases_from_form(&$question, $validation) {
        ...
        for ($i = 0; $i < $numtests; $i++){
            ...
            $stdin = qtype_sqlrunner_util::filter_crs($test_data->stdin);
            // do not filter expected. Can be non-ASCII symbol here
            $expected = ($test_data->expected);
            $extra = qtype_sqlrunner_util::filter_crs($test_data->extra);
...
//        });  // Sort by ordering field.
    ...
    public function save_question_options($question) {
        ...
        $testcasetable = "question_sqlrunner_tests";
        ...
        if ($USER->id) {
            ...
            foreach (array('datafiles' => 'datafile') as $fileset => $filearea) {
                ...
                file_save_draft_area_files($question->$fileset, $question->context->id, 'qtype_sqlrunner', $filearea, (int) $question->id,
                    $this->fileoptions);
            }
        }
    ...
    public function clean_question_form($question, $isvalidation=false) {
        ...
        if ($customised && $question->prototypetype > 0 && !$isvalidation &&
                $question->sqlrunnertype != $question->typename) {
            // Saving a new user-defined prototype.
            // Copy new type name into sqlrunnertype.
            $question->sqlrunnertype = $question->typename;
        }

        // If we're saving a new prototype, make sure its sqlrunnertype is
        ...
        if ($question->isnew && $isprototype && !$isvalidation) {
            ...
            while (true) {
                ...
                if ($suffix == '-9') {
                    throw new qtype_sqlrunner_exception('Too many templates with almost identical names');
                }
            }
            $question->sqlrunnertype = $type . $suffix;
        }
        ...
        if(!empty($question->penaltyregime)){
            ...
            $question->penaltyregime = get_config('qtype_sqlrunner', 'penaltyregime');
        }
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        foreach (array('datafile', 'samplefile') as $filetype) {
            $fs->move_area_files_to_new_context($oldcontextid,
                $newcontextid, 'qtype_sqlrunner', $filetype, $questionid);
        }
    }
    ...
    public function get_question_options($question) {
        ...
        if ($testcases = $DB->get_records('question_sqlrunner_tests',
     ...
     * be either a qtype_sqlrunner_question object or its options field ($question->options).
    ...
    public function set_inherited_fields($target, $prototype) {
        ...
        array_shift($extrafields); // Remove the extraneous qtype_sqlrunner_options entry.
     ...
     * @return stdClass[] prototype rows from question_sqlrunner_options.
     */
    public static function get_all_prototypes($courseid) {
        ...
        $rows = $DB->get_records_sql("
                SELECT qco.*
                  FROM {question_sqlrunner_options} qco
     ...
     *
     * @param string $sqlrunnertype prototype name.
     ...
     *
     * @param stdClass $question a row from either the question or question_sqlrunner_options tables.
    ...
    protected static function update_question_text_maybe($prototype) {
        if ($prototype->prototypetype == 1) { // Built-in prototype.
            $stringname = 'qtype_' . $prototype->sqlrunnertype;
    ...
    // fields not from the question_sqlrunner_options table) need handling here.
    ...
    public function delete_question($questionid, $contextid) {
        ...
        $question = $DB->get_record('question_sqlrunner_options', array('questionid' => $questionid));

        $this->notify_prototype_children_if_any($question);
        $success = $DB->delete_records('question_sqlrunner_tests', array('questionid' => $questionid));
    ...
    private function notify_prototype_children_if_any($question) {
        ...
        if (!empty($question->prototypetype)) {
            $typename = $question->sqlrunnertype;
            $children = $DB->get_records('question_sqlrunner_options', array('prototypetype' => 0, 'sqlrunnertype' => $typename));
    ...
    public function attachments_required_options() {
        return array(
            0 => sqlrunner_str('attachmentsoptional'),
            1 => '1',
            2 => '2',
            3 => '3'
        );
     ...
     *
     * Overrides default since sqlrunner questions contain a list of testcases,
     * not a list of answers.
     *
     */
    public function import_from_xml($data, $question, qformat_xml $format, $extra=null) {
        if ($extra != null) {
            throw new coding_exception("sqlrunner:import_from_xml: unexpected 'extra' parameter");
        }
    ...
    // export only the sqlrunner extra fields that are not inherited or that
    // are not equal in value to the field from the prototype.
    public function export_to_xml($question, qformat_xml $format, $extra=null) {
        global $COURSE;
        if ($extra !== null) {
            throw new coding_exception("sqlrunner:export_to_xml: Unexpected parameter");
        }
        ...
        $qtype = $question->options->sqlrunnertype;
        ...
        $datafiles = $fs->get_area_files(
                $contextid, 'qtype_sqlrunner', 'datafile', $question->id);
        ...
        $sampleanswerfiles = $fs->get_area_files(
                $contextid, 'qtype_sqlrunner', 'samplefile', $question->id);
