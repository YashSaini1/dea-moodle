...
/**
 * Unit tests for sqlrunner's jobe sandbox class.
 *
...
class qtype_sqlrunner_jobesandbox_testcase extends qtype_sqlrunner_testcase {

    public function test_fail_with_bad_key() {
        $this->check_sandbox_enabled('jobesandbox');
        if (!get_config('qtype_sqlrunner', 'jobe_apikey_enabled')) {
            $this->markTestSkipped("Jobe API key security disabled: test skipped");
        }
        set_config('jobe_apikey', 'no-such-key-we-hope', 'qtype_sqlrunner');
        $sandbox = new qtype_sqlrunner_jobesandbox();
    ...
    public function test_jobesandbox_python3_good() {
        ...
        $this->assertEquals(qtype_sqlrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_sqlrunner_sandbox::RESULT_SUCCESS, $result->result);
    ...
    public function test_jobesandbox_python3_bad() {
        ...
        $result = $sandbox->execute($source, 'python3', '');
        $this->assertEquals(qtype_sqlrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_sqlrunner_sandbox::RESULT_COMPILATION_ERROR, $result->result);
        $sandbox->close();
    }
...
";
    ...
    public function test_jobesandbox_python3_timeout() {
        ...
        $result = $sandbox->execute($source, 'python3', '');
        $this->assertEquals(qtype_sqlrunner_sandbox::OK, $result->error);
        $this->assertEquals(qtype_sqlrunner_sandbox::RESULT_TIME_LIMIT, $result->result);
...
EOCODE;
    ...
    public function test_limits_enforced() {
        $this->check_sandbox_enabled('jobesandbox');
        if (!get_config('qtype_sqlrunner', 'jobe_limits_enforced')) {
