...
/**
 * Unit tests for the sqlrunner question type.
 *
...
class qtype_sqlrunner_walkthrough_testcase extends qbehaviour_walkthrough_test_base {
    ...
    public function test_adaptive() {
        $q = test_question_maker::make_question('sqlrunner', 'sqr');
        ...
        $this->check_current_output(
                new question_pattern_expectation('/' .
                        preg_quote(get_string('noerrorsallowed', 'qtype_sqlrunner') . '/'))
              );
    ...
    public function test_view_hidden_testcases_capability() {
        ...
        $this->assertTrue(has_capability('qtype/sqlrunner:viewhiddentestcases', $coursecontext));
        ...
        role_change_permission($DB->get_field('role', 'id', ['shortname' => 'editingteacher']),
                $coursecontext, 'qtype/sqlrunner:viewhiddentestcases', CAP_PREVENT);
        $this->assertFalse(has_capability('qtype/sqlrunner:viewhiddentestcases', $coursecontext));
    ...
    public function test_partial_marks() {
        $q = test_question_maker::make_question('sqlrunner', 'sqr_part_marks');
        $this->start_attempt_at_question($q, 'adaptive', 1, 1);
    ...
    public function test_grading_template_output() {
        $q = test_question_maker::make_question('sqlrunner', 'sqrnoprint');
