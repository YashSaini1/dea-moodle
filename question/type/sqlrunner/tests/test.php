...
/**
 * Base testcase for sqlrunner tests.
...
class qtype_sqlrunner_testcase extends advanced_testcase {
    ...
    public static function setup_test_sandbox_configuration(): void {
        global $CFG, $USER;
        require($CFG->dirroot . '/question/type/sqlrunner/tests/fixtures/test-sandbox-config-dist.php');
        $localconfig = $CFG->dirroot . '/question/type/sqlrunner/tests/fixtures/test-sandbox-config.php';
    ...
    protected function check_language_available($language): void {
        if (qtype_sqlrunner_sandbox::get_best_sandbox($language, true) === null) {
    ...
    public function make_question($question) {
        try {
            $q = test_question_maker::make_question('sqlrunner', $question);
        } catch (qtype_sqlrunner_missing_question_type $ex) {
    ...
    protected function check_sandbox_enabled($sandbox): void {
        if (!get_config('qtype_sqlrunner', $sandbox . '_enabled')) {
