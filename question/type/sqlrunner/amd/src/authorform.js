...
define(['jquery', 'qtype_sqlrunner/userinterfacewrapper', 'core/str'], function($, ui, str) {
    ...
    function initEditForm() {
        var typeCombo = $('#id_sqlrunnertype'),
         ...
         * set up by code in edit_sqlrunner_form.php (q.v.) but those event
        ...
        function copyFieldsFromQuestionType(newType, response) {
            ...
            str.get_string('sqlrunner_question_type', 'qtype_sqlrunner').then(function (s) {
         ...
         * @param {string} questionType The sqlrunner (sub) question type.
        ...
        function reportError(questionType, error) {
            ...
            str.get_string('prototype_error', 'qtype_sqlrunner').then(function(s) {
                var errorMessage = s + "\n";
                errorMessage += error + '\n';
         ...
         * @param {string} sqlrunner_descr The language string to introduce
         * the detail.
         ...
         */
        function detailsHtml(title, sqlrunner_descr, html) {

            var resultHtml = '<p class="question-type-details-header">';
            resultHtml += sqlrunner_descr;
            resultHtml += title + '</p>\n' + html;
            return resultHtml;
        ...
        function langStringAlert(key, extra) {
            ...
            str.get_string(key, 'qtype_sqlrunner').then(function(s) {
         ...
         * sqlrunner question type currently selected by the combobox.
         */
        function loadCustomisationFields() {
            ...
            if (newType !== '' && newType !== 'Undefined') {
                ...
                $.getJSON(M.cfg.wwwroot + '/question/type/sqlrunner/ajax.php',
                    {
                        qtype: newType,
                        courseid: courseId,
                    ...
                    str.get_string('ajax_error', 'qtype_sqlrunner').then(function(s) {
        ...
        function checkForBrokenQuestion() {
            ...
            if (brokenQuestionMessage !== '') {
                ...
                $('#id_qtype_sqlrunner_error_div').append(messagePara);
            }
        }
        ...
        if (prototypeType.prop('value') == 1) {
            ...
            str.get_string('proceed_at_own_risk', 'qtype_sqlrunner').then(function(s) {
                alert(s);
            });
        ...
        if (!isCustomised) {
            ...
            str.get_string('info_unavailable', 'qtype_sqlrunner').then(function(s) {
        ...
        customise.on('change', function() {
            ...
            if (isCustomised) {
                ...
                str.get_string('confirm_proceed', 'qtype_sqlrunner').then(function(s) {
                    if (window.confirm(s)) {
                        setCustomisationVisibility(false);
                    } else {
        ...
        typeCombo.on('change', function() {
            if (customise.prop('checked')) {
                ...
                str.get_string('question_type_changed', 'qtype_sqlrunner').then(function (s) {
                    if (window.confirm(s)) {
