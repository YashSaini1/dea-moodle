...
define(['jquery'], function ($) {
    ...
    function loadQuestionText(qid, divId, questionFilename) {
        ...
        qDiv = '<div>\n' +
            '<button type="button" class="qtype_sqlrunner_previous">Previous</button>\n' +
            '<button type="button" class="qtype_sqlrunner_next">Next</button>' +
            '&nbsp; &nbsp;' +
            '<span>Page: <span class="qtype_sqlrunner_pagenum"></span> / <span ' +
            'class="qtype_sqlrunner_numpages"></span></span>\n' +
            '</div>\n' +
            '<canvas id="qtype_sqlrunner_problemspec_' + qid + '"></canvas>\n' +
            '</div>\n';
        questionTextDiv.append(qDiv);
        canvas = document.getElementById('qtype_sqlrunner_problemspec_' + qid);
        next = $('#' + divId + ' button.qtype_sqlrunner_next');
        previous = $('#' + divId + ' button.qtype_sqlrunner_previous');
        pageNumSpan = $('#' + divId + ' span.qtype_sqlrunner_pagenum');
        numPagesSpan = $('#' + divId + ' span.qtype_sqlrunner_numpages');
        ctx = canvas.getContext('2d');
        next.click(onNextPage);
        previous.click(onPrevPage);
        $.getJSON(M.cfg.wwwroot + '/question/type/sqlrunner/problemspec.php',
                {
                    questionid: qid,
