 ...
 */
define("qtype_sqlrunner/ui_table",["jquery"],(function($){function TableUi(textareaId,width,height,uiParams){if(this.textArea=$(document.getElementById(textareaId)),this.readOnly=this.textArea.prop("readonly"),this.tableDiv=null,this.uiParams=uiParams,!uiParams.num_columns||!uiParams.num_rows)return this.fail=!0,void(this.failString="table_ui_missingparams");this.fail=!1,this.lockedCells=uiParams.locked_cells||[],this.hasHeader=!!(uiParams.column_headers&&uiParams.column_headers.length>0),this.hasRowLabels=!!(uiParams.row_labels&&uiParams.row_labels.length>0),this.numDataColumns=uiParams.num_columns,this.rowsPerCell=uiParams.lines_per_cell||2,this.totNumColumns=this.numDataColumns+(this.hasRowLabels?1:0),this.columnWidths=this.computeColumnWidths(),this.reload()}return TableUi.prototype.computeColumnWidths=function(){var defaultWidth=Math.trunc(100/this.totNumColumns),columnWidths=[];if(this.uiParams.column_width_percents&&this.uiParams.column_width_percents.length>0)return this.uiParams.column_width_percents;if(Array.prototype.fill)return new Array(this.totNumColumns).fill(defaultWidth);for(var i=0;i<this.totNumColumns;i++)columnWidths.push(defaultWidth);return columnWidths},TableUi.prototype.isLockedCell=function(row,col){for(var i=0;i<this.lockedCells.length;i++)if(this.lockedCells[i][0]==row&&this.lockedCells[i][1]==col)return!0;return!1},TableUi.prototype.getElement=function(){return this.tableDiv},TableUi.prototype.failed=function(){return this.fail},TableUi.prototype.failMessage=function(){return this.failString},TableUi.prototype.sync=function(){var serialisation=[],empty=!0;$(this.tableDiv).find("table tbody tr").each((function(){var rowValues=[];$(this).find("textarea").each((function(){var cellVal=$(this).val();rowValues.push(cellVal),cellVal&&(empty=!1)})),serialisation.push(rowValues)})),empty?this.textArea.val(""):this.textArea.val(JSON.stringify(serialisation))},TableUi.prototype.tableRow=function(iRow,preload){var html="<tr>",widthIndex=0;this.hasRowLabels&&(widthIndex=1,html+="<th style='padding-top:8px;text-align:center;width:"+this.columnWidths[0]+"%' scope='row'>",iRow<this.uiParams.row_labels.length&&(html+=this.uiParams.row_labels[iRow]),html+="</th>");for(var iCol=0;iCol<this.numDataColumns;iCol++)html+="<td style='padding:2px;margin:0,width:"+this.columnWidths[widthIndex++]+"'%>",html+='<textarea rows="'+this.rowsPerCell+'"',html+=' style="width:100%;padding:0;resize:vertical;font-family: monospace"',this.isLockedCell(iRow,iCol)?html+=" disabled>":html+=">",iRow<preload.length&&(html+=preload[iRow][iCol]),html+="</textarea>",html+="</td>";return html+="</tr>"},TableUi.prototype.tableHeadSection=function(){var html="<thead>\n",colIndex=0;if(this.hasHeader){html+="<tr>",this.hasRowLabels&&(html+="<th style='width:"+this.columnWidths[0]+"%'></th>",colIndex+=1);for(var iCol=0;iCol<this.numDataColumns;iCol++)html+="<th style='width:"+this.columnWidths[colIndex]+"%'>",iCol<this.uiParams.column_headers.length&&(html+=this.uiParams.column_headers[iCol]),colIndex++,html+="</th>";html+="</tr>\n"}return html+="</thead>\n"},TableUi.prototype.reload=function(){var preloadJson=$(this.textArea).val(),preload=[],divHtml="<div style='height:fit-content' class='qtype-sqlrunner-table-outer-div'>\n<table class='table table-bordered qtype-sqlrunner_table'>\n";if(preloadJson)try{preload=JSON.parse(preloadJson)}catch(error){return this.fail=!0,void(this.failString="table_ui_invalidjson")}try{divHtml+=this.tableHeadSection(),divHtml+="<tbody>\n";for(var num_rows_required=Math.max(this.uiParams.num_rows,preload.length),iRow=0;iRow<num_rows_required;iRow++)divHtml+=this.tableRow(iRow,preload);divHtml+="</tbody>\n</table>\n</div>",this.tableDiv=$(divHtml),this.uiParams.dynamic_rows&&this.addButtons()}catch(error){this.fail=!0,this.failString="table_ui_invalidserialisation"}},TableUi.prototype.addButtons=function(){var deleteButton=$('<button type="button"style="float:right;margin-right:6px" disabled>Delete row</button>'),t=this;this.tableDiv.append(deleteButton),deleteButton.click((function(){var numRows=t.tableDiv.find("table tbody tr").length,lastRow=t.tableDiv.find("tr:last");numRows>t.uiParams.num_rows&&lastRow.remove(),lastRow=t.tableDiv.find("tr:last"),numRows==t.uiParams.num_rows+1&&$(this).prop("disabled",!0)}));var addButton=$('<button type="button"style="float:right;margin-right:6px">Add row</button>');t.tableDiv.append(addButton),addButton.click((function(){var lastRow,newRow;(newRow=(lastRow=t.tableDiv.find("table tbody tr:last")).clone()).find("textarea").each((function(){$(this).val("")})),lastRow.after(newRow),$(this).prev().prop("disabled",!1)}))},TableUi.prototype.resize=function(){},TableUi.prototype.hasFocus=function(){var focused=!1;return $(this.tableDiv).find("textarea").each((function(){this===document.activeElement&&(focused=!0)})),focused},TableUi.prototype.destroy=function(){this.sync(),$(this.tableDiv).remove(),this.tableDiv=null},{Constructor:TableUi}}));
