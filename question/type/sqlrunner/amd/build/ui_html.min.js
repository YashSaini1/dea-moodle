 ...
 */
define("qtype_sqlrunner/ui_html",["jquery"],(function($){function HtmlUi(textareaId,width,height,uiParams){this.textArea=$(document.getElementById(textareaId)),this.textareaId=textareaId;var srcField=uiParams.html_src||"globalextra";this.html=this.textArea.attr("data-"+srcField),this.html=this.html.replace(/___textareaId___/gm,textareaId),this.readOnly=this.textArea.prop("readonly"),this.uiParams=uiParams,this.fail=!1,this.htmlDiv=null,this.reload()}return HtmlUi.prototype.failed=function(){return this.fail},HtmlUi.prototype.failMessage=function(){return"htmluiloadfail"},HtmlUi.prototype.sync=function(){var name,serialisation={},empty=!0;this.getFields().each((function(){var value,type;type=$(this).attr("type"),name=$(this).attr("name"),value="checkbox"!==type&&"radio"!==type||$(this).is(":checked")?$(this).val():"",serialisation.hasOwnProperty(name)?serialisation[name].push(value):serialisation[name]=[value],""!==value&&(empty=!1)})),empty?this.textArea.val(""):this.textArea.val(JSON.stringify(serialisation))},HtmlUi.prototype.getElement=function(){return this.htmlDiv},HtmlUi.prototype.getFields=function(){return $(this.htmlDiv).find(".sqlrunner-ui-element")},HtmlUi.prototype.setField=function(field,value){"checkbox"===field.attr("type")||"radio"===field.attr("type")?field.prop("checked",field.val()===value):field.val(value)},HtmlUi.prototype.reload=function(){var valuesToLoad,values,i,fields,leftOvers,content=$(this.textArea).val(),outerDiv="<div style='height:fit-content' class='qtype-sqlrunner-html-outer-div' id='"+("qtype-sqlrunner-outer-div-"+this.textareaId.toString())+"'>";if(this.htmlDiv=$(outerDiv+this.html+"</div>"),this.htmlDiv.data("uiparams",this.uiParams),this.htmlDiv.data("templateparams",this.uiParams),content)try{for(var name in valuesToLoad=JSON.parse(content),leftOvers={},valuesToLoad){for(values=valuesToLoad[name],fields=this.getFields().filter("[name='"+name+"']"),leftOvers[name]=[],i=0;i<values.length;i++)i<fields.length?this.setField($(fields[i]),values[i]):leftOvers[name].push(values[i]);0===leftOvers[name].length&&delete leftOvers[name]}$.isEmptyObject(leftOvers)||this.htmlDiv.data("leftovers",leftOvers)}catch(e){this.fail=!0}},HtmlUi.prototype.resize=function(){},HtmlUi.prototype.hasFocus=function(){var focused=!1;return this.getFields().each((function(){this===document.activeElement&&(focused=!0)})),focused},HtmlUi.prototype.destroy=function(){this.sync(),$(this.htmlDiv).remove(),this.htmlDiv=null},{Constructor:HtmlUi}}));
