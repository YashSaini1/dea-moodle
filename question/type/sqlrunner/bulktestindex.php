...
$PAGE->set_url('/question/type/sqlrunner/bulktestindex.php');
$PAGE->set_context($context);
$PAGE->set_title(sqlrunner_str('bulktestindextitle'));
...
// Display.
echo $OUTPUT->header();
echo $OUTPUT->heading(sqlrunner_str('sqlrunnercontexts'));

// Find in which contexts the user can edit questions.
$questionsbycontext = $bulktester->get_num_sqlrunner_questions_by_context();
$availablequestionsbycontext = array();
foreach ($questionsbycontext as $contextid => $numsqlrunnerquestions) {
    ...
    if (has_capability('moodle/question:editall', $context)) {
        ...
        $availablequestionsbycontext[$name] = array(
            'contextid' => $contextid,
            'numquestions' => $numsqlrunnerquestions);
    }
}
...
if (count($availablequestionsbycontext) == 0) {
    ...
    foreach ($availablequestionsbycontext as $name => $info) {
        ...
        $numsqlrunnerquestions = $info['numquestions'];

        $testallurl = new moodle_url('/question/type/sqlrunner/bulktest.php', array('contextid' => $contextid));
        $testalllink = html_writer::link($testallurl,
                sqlrunner_str('bulktestallincontext'),
                array('title' => sqlrunner_str('testalltitle'),
                       'style' => $buttonstyle));
        $expandlink = html_writer::link('#expand',
            sqlrunner_str('expand'),
                array('class' => 'expander',
                      'title' => sqlrunner_str('expandtitle'),
                      'style' => $buttonstyle));
        $litext = $name . ' (' . $numsqlrunnerquestions . ') ' . $testalllink . ' ' . $expandlink;
        if (strpos($name, 'Quiz:') === 0) {
            $class = 'bulktest sqlrunner context quiz';
        } else {
            $class = 'bulktest sqlrunner context normal';
        }
        ...
        foreach ($categories as $cat) {
            if ($cat->count > 0) {
                $url = new moodle_url('/question/type/sqlrunner/bulktest.php',
                ...
                echo html_writer::tag('li', $link,
                        array('title' => sqlrunner_str('testallincategory')));
            }
        }
    ...
    if (has_capability('moodle/site:config', context_system::instance())) {
        echo html_writer::tag('p', html_writer::link(
                new moodle_url('/question/type/sqlrunner/bulktestall.php'), sqlrunner_str('bulktestrun')));
    }
}
