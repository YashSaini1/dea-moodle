...
$PAGE->set_url('/question/type/sqlrunner/prototypeusage.php');
$PAGE->set_context($context);
$PAGE->set_title(sqlrunner_str('prototypeusage'));
...
// Start display.
echo $OUTPUT->header();
echo $OUTPUT->heading(sqlrunner_str('prototypeusage', $coursename));

$coursecontext = context::instance_by_id($coursecontextid);
if (!has_capability('moodle/question:editall', $coursecontext)) {
    echo html::tag('p', sqlrunner_str('unauthorisedbulktest'));
} else {
    $questions = $bulktester->get_all_sqlrunner_questions_in_context($coursecontextid);
    $prototypequestions = qtype_sqlrunner::get_all_prototypes($courseid);
    $prototypes = [];
    foreach ($prototypequestions as $prototype) {
        $prototypes[$prototype->sqlrunnertype] = $prototype;
    }
