 ...
 * @package mod_hvp
 */
class sql_view_assets extends \mod_hvp\view_assets {
    ...
    public function __construct($cm, $course, $options = []) {
        ...
        $this->settings    = sql_hvp_get_core_assets(\context_module::instance($cm->id));
        $this->jsrequires  = [];
        $this->cssrequires = [];

        global $PAGE;
        $PAGE->requires->js_call_amd('local_sql/track_video_viewed', 'init', ['cmid' => $cm->id]);
