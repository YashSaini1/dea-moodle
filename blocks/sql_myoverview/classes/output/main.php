 ...
 */
namespace block_sql_myoverview\output;
...
class main implements renderable, templatable {
    ...
    public function __construct($grouping, $sort, $view, $paging, $customfieldvalue = null) {
        global $CFG;
        // Get plugin config.
        $config = get_config('block_sql_myoverview');
        ...
        if ($sort) {
            ...
            $this->sort = BLOCK_SQL_MYOVERVIEW_SORTING_SHORTNAME;
        } else {
            $this->sort = BLOCK_SQL_MYOVERVIEW_SORTING_SORTORDER;
        }
        // In case sorting remembered is shortname and display extended course names not checked,
        // we should revert sorting to title.
        if (!$CFG->courselistshortnames && $sort == BLOCK_SQL_MYOVERVIEW_SORTING_SHORTNAME) {
            $this->sort = BLOCK_SQL_MYOVERVIEW_SORTING_TITLE;
        }

        // Check and remember the given view.
        $this->view = $view ? $view : BLOCK_SQL_MYOVERVIEW_VIEW_CARD;

        // Check and remember the given page size, `null` indicates no page size set
        // while a `0` indicates a paging size of `All`.
        if (!is_null($paging) && $paging == BLOCK_SQL_MYOVERVIEW_PAGING_ALL) {
            $this->paging = BLOCK_SQL_MYOVERVIEW_PAGING_ALL;
        } else {
            $this->paging = $paging ? $paging : BLOCK_SQL_MYOVERVIEW_PAGING_12;
        }

        // Check and remember if the course categories should be shown or not.
        if (!$config->displaycategories) {
            $this->displaycategories = BLOCK_SQL_MYOVERVIEW_DISPLAY_CATEGORIES_OFF;
        } else {
            $this->displaycategories = BLOCK_SQL_MYOVERVIEW_DISPLAY_CATEGORIES_ON;
        }
    ...
    private function get_fallback_grouping($config) {
        if ($config->displaygroupingall == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_ALL;
        }
        if ($config->displaygroupingallincludinghidden == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_ALLINCLUDINGHIDDEN;
        }
        if ($config->displaygroupinginprogress == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_INPROGRESS;
        }
        if ($config->displaygroupingfuture == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_FUTURE;
        }
        if ($config->displaygroupingpast == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_PAST;
        }
        if ($config->displaygroupingfavourites == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_FAVOURITES;
        }
        if ($config->displaygroupinghidden == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_HIDDEN;
        }
        if ($config->displaygroupingcustomfield == true) {
            return BLOCK_SQL_MYOVERVIEW_GROUPING_CUSTOMFIELD;
        }
    ...
    public function set_available_layouts() {
        ...
        if ($config = get_config('block_sql_myoverview', 'layouts')) {
            $this->layouts = explode(',', $config);
        } else {
            $this->layouts = array(BLOCK_SQL_MYOVERVIEW_VIEW_CARD);
        }
    }
    ...
    public function format_layout_for_export($layoutname) {
        ...
        $layout->name = get_string($layoutname, 'block_sql_myoverview');
        $layout->active = $this->view == $layoutname ? true : false;
        $layout->arialabel = get_string('aria:' . $layoutname, 'block_sql_myoverview');
    ...
    public function get_customfield_values_for_export() {
        ...
        $customfieldactive = ($this->grouping === BLOCK_SQL_MYOVERVIEW_GROUPING_CUSTOMFIELD);
        $ret = [];
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $nocoursesurl = $output->image_url('courses', 'block_sql_myoverview')->out();
        ...
        $selectedcustomfield = '';
        if ($this->grouping == BLOCK_SQL_MYOVERVIEW_GROUPING_CUSTOMFIELD) {
            ...
            if (!$selectedcustomfield) {
                $this->grouping = $this->get_fallback_grouping(get_config('block_sql_myoverview'));
                if ($this->grouping == BLOCK_SQL_MYOVERVIEW_GROUPING_CUSTOMFIELD) {
        ...
        $sort = '';
        if ($this->sort == BLOCK_SQL_MYOVERVIEW_SORTING_SHORTNAME){
            $sort = 'shortname';
        } elseif ($this->sort == BLOCK_SQL_MYOVERVIEW_SORTING_SORTORDER) {
            $sort = BLOCK_SQL_MYOVERVIEW_SORTING_SORTORDER;
        } else {
            $sort = $this->sort == BLOCK_SQL_MYOVERVIEW_SORTING_TITLE ? 'fullname' : 'ul.timeaccess desc';
        }
