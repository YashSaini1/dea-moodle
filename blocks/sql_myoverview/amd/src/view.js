...
import $ from 'jquery';
import * as Repository from 'block_sql_myoverview/repository';
...
import SELECTORS from 'block_sql_myoverview/selectors';
import * as PagedContentEvents from 'core/paged_content_events';
import * as Aria from 'core/aria';
import {debounce} from 'core/utils';

const TEMPLATES = {
    COURSES_CARDS: 'block_sql_myoverview/view-cards',
    COURSES_LIST: 'block_sql_myoverview/view-list',
    COURSES_SUMMARY: 'block_sql_myoverview/view-summary',
...
const DEFAULT_PAGED_CONTENT_CONFIG = {
    ...
    persistentLimitKey: 'block_sql_myoverview_user_paging_preference'
};
...
const setCourseHiddenState = (courseId, status) => {
    ...
    return Repository.updateUserPreferences({
        preferences: [
            {
                type: 'block_sql_myoverview_hidden_course_' + courseId,
                value: status
            }
        ]
    });
...
export const init = root => {
    ...
    if (!root.attr('data-init')) {
        ...
        namespace = "block_sql_myoverview_" + root.attr('id') + "_" + Math.random();
