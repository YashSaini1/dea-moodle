 ...
 */
function xmldb_block_sql_myoverview_upgrade($oldversion) {
    ...
    if ($oldversion < 2024013103) {
        require_once ($CFG->dirroot.'/blocks/sql_myoverview/lib.php');
        $DB->execute("UPDATE {user_preferences} SET value = ? WHERE name = ? AND value != ?",[
            BLOCK_SQL_MYOVERVIEW_SORTING_SORTORDER, 'block_sql_myoverview_user_sort_preference', BLOCK_SQL_MYOVERVIEW_SORTING_LASTACCESSED
        ]);
    }
