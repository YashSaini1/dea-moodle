...
$attemptobj = theme_sql_quiz_create_attempt_handling_errors($attemptid, $cmid);
...
// $USER->slot is current processed slot, which setting up in behaviour
if (isset($USER->slot)){
    $slot = $USER->slot;
    unset($USER->slot);
    if(count($attemptobj->get_slots()) < $slot + 1){
        $url = $attemptobj->view_url();
    } else {
        $url =$attemptobj->attempt_url($slot + 1);
    }
    redirect($url);
}

if (!\theme_sql\sql_access_to_premium::has_access(($nextpage + 1), $COURSE->id) && $next) {
    $url = $attemptobj->attempt_url();
    $url->param('page', $thispage);
    $url->param('showupdatepopup', 1);
    redirect($url);
}
