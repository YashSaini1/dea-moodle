...
$block      = optional_param('block', 0, PARAM_INT);
...
$norole = get_string("no") . ' ' . get_string("role");
...
if ($confirmuser and confirm_sesskey()) {
    ...
    redirect($returnurl);
} else if ($unblock and confirm_sesskey()) {
    require_capability('moodle/user:delete', $sitecontext);

    if ($user = $DB->get_record('user', array('id' => $unblock, 'mnethostid' => $CFG->mnet_localhost_id, 'deleted' => 0))) {
        login_unlock_account($user);

        $DB->set_field('user', 'waitonboarding', 0, array('id' => $unblock));
        \core\session\manager::kill_user_sessions($user->id);
    }
    redirect($returnurl);
} else if ($block and confirm_sesskey()) {
    require_capability('moodle/user:delete', $sitecontext);

    if ($user = $DB->get_record('user', array('id' => $block, 'mnethostid' => $CFG->mnet_localhost_id, 'deleted' => 0))) {
        login_unlock_account($user);

        $DB->set_field('user', 'waitonboarding', 1, array('id' => $block));
        \core\session\manager::kill_user_sessions($user->id);
    }
    redirect($returnurl);
}
...
$users = get_users_listing_local($sort, $dir, $page*$perpage, $perpage, '', '', '',
...
if (!$users) {
    ...
    foreach ($users as $user) {
        ...
        if (has_capability('moodle/user:update', $sitecontext)) {
            if (!role_manager::is_admin($user)) {
                if ($user->waitonboarding == 1) {
                    $url = new moodle_url($returnurl, array('unblock'=>$user->id, 'sesskey'=>sesskey()));
                    $buttons[] = html_writer::link($url, $OUTPUT->pix_icon('t/unlock', 'Unblock Onboarding'));
                } else {
                    $url = new moodle_url($returnurl, array('block'=>$user->id, 'sesskey'=>sesskey()));
                    $buttons[] = html_writer::link($url, $OUTPUT->pix_icon('t/lock', 'Block Onboarding'));
                }
            }
        }
        ...
        $context = context_system::instance();
        $user_roles = get_user_roles($context, $user->id, false, 'r.name');
        $role_display = '';
        if (!empty($user_roles)) {
            $role_names = [];
