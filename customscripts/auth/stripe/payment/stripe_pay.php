...
if (!get_config('local_ab_testing', 'enable')){
    return;
}
...
/////////////////////// Return if coaching
if (core::is_coaching_page($page_type)){
    return;
}
...
try {
    ...
    \auth_stripe\core::error('PAYMENT ERROR - '.$e->getMessage(), $e);
    $result['status'] = 'error';
...
if (!$payed_successfully) {
    ...
    $result['redirect_url'] = $CFG->wwwroot.$page.'?'.
        premium_price_annual_test::build_utm_query_url(
            premium_price_annual_test::get_user_campaign(),
            $page,
            \auth_stripe\util\util::format_premium_period($price)
        );
}
$send_result();