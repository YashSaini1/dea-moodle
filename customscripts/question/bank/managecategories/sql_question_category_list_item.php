 ...
 */
class sql_question_category_list_item extends \qbank_managecategories\question_category_list_item {
    ...
    public function set_icon_html($first, $last, $lastitem) : void {
        global $CFG;
        $category = $this->item;
        if (!empty($category->hide_category_info)) return;

        $url = new moodle_url('/question/bank/managecategories/category.php', ($this->parentlist->pageurl->params() + ['edit' => $category->id]));
    ...
    public function item_html($extraargs = []) : string {
        ...
        if (!empty($category->hide_category_info)){
            return '';
        }
    ...
    public function to_html($indent = 0, $extraargs = array()) {
        ...
        $childrenhtml = (join('', $this->icons)).(($childrenhtml != '') ? ("\n".$childrenhtml) : '');
        if (!empty($this->item->hide_category_info)){
            return $childrenhtml;
        }
        return $this->item_html($extraargs).'&nbsp;'.$childrenhtml;
    }
}
