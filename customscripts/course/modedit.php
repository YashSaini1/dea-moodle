...
if ($module->name == 'quiz'){
    if (!empty($add)){
        ...
        $data->completionusegrade = '';
        $data->completionpassgrade = 0;
    }
    require_once("$CFG->customscripts/course/quiz_mod_form.php");
} elseif ($module->name == 'hvp') {
    if (!empty($add)){
        $data->completion = COMPLETION_TRACKING_AUTOMATIC;
        $data->completionview = 1;
        $data->completionusegrade = '';
    ...
    require_once("$CFG->customscripts/course/hvp_mod_form.php");
} elseif ($module->name == 'url') {
    if (!empty($add)){
    ...
    require_once("$CFG->customscripts/course/url_mod_form.php");
} elseif ($module->name == 'resource') {
    require_once("$CFG->customscripts/course/resource_mod_form.php");
} elseif (file_exists($modmoodleform)) {
...
if($module->name == 'quiz'){
    $mform = new sql_mod_quiz_mod_form($data, $cw->section, $cm, $course);
} elseif($module->name == 'hvp'){
    $mform = new sql_mod_hvp_mod_form($data, $cw->section, $cm, $course);
} elseif($module->name == 'resource'){
    $mform = new sql_mod_resource_mod_form($data, $cw->section, $cm, $course);
} elseif($module->name == 'url'){
    $mform = new sql_mod_url_mod_form($data, $cw->section, $cm, $course);
} else {
...
if ($mform->is_cancelled()) {
    ...
    if (method_exists($mform, 'post_save')){
        $mform->post_save($fromform);
    }
