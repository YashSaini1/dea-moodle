...
class external_api {
    ...
    public static function call_external_function($function, $args, $ajaxonly=false) {
        ...
        try {
            ...
            foreach ($externalfunctioninfo->parameters_desc->keys as $key => $desc) {
                if ($desc instanceof external_value && $desc->type == PARAM_LANG) {
                    if (isset($args[$key]) && !array_key_exists($args[$key], $installedlangs)) {
                        $langcode = strpos($args[$key], '_') !== false ? substr($args[$key], 0, 2) : $args[$key];
