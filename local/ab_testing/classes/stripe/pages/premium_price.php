...
use local_ab_testing\stripe\premium_price_test_product;
...
class premium_price {
    ...
    public static function payment_page($authplugin, $page_type, $token = '', $coupon = null){
        ...
        if (!empty($token)){
            $product = premium_price_test_product::get_by_page($page_type);
            $prices = price::get_all_by_token($token);
            $product_cards = premium_price_test_product::render_prices($product, $prices);
        } else {
            $products = premium_price_test_product::get_all_by_page($page_type);
            $product_cards = premium_price_test_product::render_product_cards($products);
        }
