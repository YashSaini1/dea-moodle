...
class file_logger {
    ...
    public static function createLogFile(){
        ...
        static::$log_file = static::$log_path . "/logs/{$time}-log.txt";

        //Check if directory /logs exists
        if(!file_exists(static::$log_path . '/logs')){
            mkdir(static::$log_path . '/logs', 0777, true);
        }
    ...
    public static function writeLog($args = []){
        //Create the log file
        if (!static::$filecreated){
            static::createLogFile($args['context']);
        }
        ...
        if (!empty($args['context'])){
            $contextLog = 'Context: '.json_encode($args['context']).PHP_EOL;
        }
        ...
        if ($args['severity'] == 'ERROR'){
            $additional = print_r($args['bt'], 1);
        }
        // Write time, url, & message to end of file
        fwrite(static::$file, "{$timeLog}: {$messageLog}".PHP_EOL.$contextLog.$additional);
